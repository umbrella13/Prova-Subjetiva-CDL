...
# assumindo que o arquivo "documentos" é o Documentos do windows
    ...
    for char in texto:
        if char.isdigit():
            ...
            if numero_str:
                ...
                numero_str = ""
    # caso a string termine com dígitos, adiciona o número final à lista
    if numero_str:
        nums.append(int(numero_str))
    return nums
...
# verifica se os dois arquivos existem e permite seleção entre eles
if os.path.exists(caminho_pdf) and os.path.exists(caminho_docx):
    print("Foram encontrados os arquivos .pdf e .docx.")
    escolha = input("Digite 'pdf' para ler o arquivo PDF ou 'docx' para ler o arquivo DOCX: ").strip().lower()
    if escolha == "pdf":
        print(f"arquivo .pdf selecionado: {caminho_pdf}")
        ler_pdf(caminho_pdf)
    elif escolha == "docx":
        print(f"arquivo .docx selecionado: {caminho_docx}")
